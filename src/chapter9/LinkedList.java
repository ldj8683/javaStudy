...
public class LinkedList<E> {
	...
	// 생성자
	public LinkedList() {
		head = crnt = null;	// 노드가 하나도 없는 비어있는 연결리스트 생성
	}
	...
	public void addLast(E obj) {
		if(head == null)	// 리스트가 비었는지를 확인
			...
			Node<E> ptr = head;		// 머리 노드로 초기화
			while (ptr.next != null)	// 꼬리 노드가 나올때까지 반복하는 반복문 생성
				ptr = ptr.next;			// 반복을 하면서 계쏙해서 다음 노드를 가리키며 스캔을 함
			ptr.next = crnt = new Node<E>(obj, null);	// new Node<E>(obj, null)으로 삽입할 노드를 생성
					// 생성한 노드의 데이터는 obj가 되고 뒤쪽 포인터가 참조하는 곳은 null
					// ptr.next가 참조하는 곳이 새로 삽입된 노드으를 가리키도록 해줍니다.
		}
	}
	...
	public void removeLast() {
		if( head != null ) {
			if ( head.next == null )	// 노드가 하나만 있으면 머리 노드를 삭제
				...
				crnt = pre;				// 선택 노드 crnt는 삭제 후의 꼬리 노드를 참조하게 됩니다.
			}
		}
	}
	...
	public void printCurrentNode() {
		if(crnt == null)
			...
			System.out.println(crnt.data);	// 선택 노드가 가리키고있는 데이터를 출력
	}
