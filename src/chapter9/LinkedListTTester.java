...
public class LinkedListTTester {
	static Scanner stdIn = new Scanner(System.in);

	// 데이터 (회원번호+이름)
	static class Data {
		static final int NO = 1; // 번호를 입력
		static final int NAME = 2; // 이름을 입력
		...
		void scanData(String guide, int sw) {
			...
			// 입력을 받아서 해당 메뉴의 번호와 그 곳에 입력할 이름 데이터를 입력해서 변수에 저장
			if ((sw & NO) == NO) {
				System.out.print("번호：");
				no = stdIn.nextInt();
			}
	...
	enum Menu {
		...
		NEXT("선택 노드를 하나 뒤쪽으로 이동"), PRINT_CRNT("선택 노드를 출력"), PURGE_NO("같은 번호의 노드를 삭제"), PURGE_NAME("같은 이름의 노드를 삭제"),
		RETRIEVE("임의의 노드를 출력"), DUMP("모든 노드를 출력"), TERMINATE("종료");
	...
	public static void main(String[] args) {
		...
		LinkedListT<Data> list = new LinkedListT<Data>(); // 리스트를 생성
